...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            train_losses_mean = sum(train_result.losses) / len(train_result.losses)
            test_losses_mean = sum(test_result.losses) / len(test_result.losses)
            actual_num_epochs += 1
            ...
            if early_stopping is not None and epoch > 0 and test_loss[epoch] >= test_loss[epoch-1]:
...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        y_scores = y_scores.reshape(-1, y_scores.shape[2])
        y = y.reshape(-1)
